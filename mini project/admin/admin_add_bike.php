...
$uploadError = "";
$bikeNumberError = ""; // Variable to hold bike number error message
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM bike WHERE bike_no = ?");
    $stmt->bind_param("s", $bikeNumber);
    $stmt->execute();
    ...
    if ($count > 0) {
        $bikeNumberError = "* Bike number already exists";
    } else {
...
<body>
    <div class="container">
        ...
        <form action="admin_add_bike.php" method="POST" enctype="multipart/form-data">
            ...
            <?php if (!empty($bikeNumberError)): ?>
                <p class="error"><?php echo $bikeNumberError; ?></p>
            <?php endif; ?>
