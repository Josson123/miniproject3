...
if(isset($_POST['confirm_pickup'])) {
    ...
    if($stmt->execute()) {
        // Update the booking status to indicate pickup
        $update_pickup = "UPDATE booking SET pickup_status = 1 WHERE booking_no = ?";
        $update_stmt = $conn->prepare($update_pickup);
        $update_stmt->bind_param("s", $booking_no);
        if($update_stmt->execute()) {
            echo "<script>
                alert('Pickup successful!');
                var row = document.getElementById('pickup-row-" . $booking_no . "');
                if(row) {
                    row.remove();
                }
            </script>";
...
if(isset($_POST['confirm_return'])) {
    ...
    try {
        ...
        // Update bike status
        $update_bike = "UPDATE bike SET booking_status = 0 WHERE bike_no = ?";
        $stmt_bike = $conn->prepare($update_bike);
        $stmt_bike->bind_param("s", $bike_no);
        $stmt_bike->execute();
        
        // Commit transaction
        $conn->commit();
        
        // Set the success flag and booking number
        $return_success = true;
        $removed_booking_no = $booking_no;
        
    } catch (Exception $e) {
        // Rollback transaction on error
        $conn->rollback();
        echo "<script>alert('Error processing return: " . $e->getMessage() . "');</script>";
    }
}

// Fetch today's pickups
$pickup_query = "SELECT b.booking_no, b.pickup_date, b.user_name, b.otp, bk.bike_name, b.dropoff_date, b.pickup_status 
                 FROM booking b 
                 INNER JOIN bike bk ON b.bike_no = bk.bike_no
                 WHERE DATE(b.pickup_date) = CURDATE() AND b.pickup_status = 0";
$pickup_result = $conn->query($pickup_query);

// Fetch today's drop-offs
$dropoff_query = "SELECT b.booking_no, b.dropoff_date, b.user_name, bk.bike_name, b.bike_no 
                  FROM booking b 
                  INNER JOIN bike bk ON b.bike_no = bk.bike_no
            ...
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <?php while($row = $dropoff_result->fetch_assoc()): ?>
                            <tr id="dropoff-row-<?php echo $row['booking_no']; ?>">
                                <td><?php echo $row['booking_no']; ?></td>
                                <td><?php echo $row['dropoff_date']; ?></td>
                                <td><?php echo $row['bike_name']; ?></td>
                                <td><?php echo $row['user_name']; ?></td>
                                <td>
                                    <form method="POST">
                                        <input type="hidden" name="booking_no" value="<?php echo $row['booking_no']; ?>">
                                        <input type="hidden" name="bike_no" value="<?php echo $row['bike_no']; ?>">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var row = document.getElementById('dropoff-row-<?php echo $removed_booking_no; ?>');
        if(row) {
            row.remove();
        }
    });
