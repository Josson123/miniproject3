...
if (isset($_POST['confirm_booking'])) {
    ...
    try {
        ...
        // Get the latest serial number for today
        $serial_query = "SELECT MAX(SUBSTRING(booking_no, -2)) as latest_serial 
                        FROM booking 
                        WHERE DATE(booking_date) = CURDATE()";
        $serial_result = $conn->query($serial_query);
        $serial_row = $serial_result->fetch_assoc();
        $serial_number = str_pad(($serial_row['latest_serial'] === null ? 0 : intval($serial_row['latest_serial']) + 1), 2, '0', STR_PAD_LEFT);
        ...
        $stmt->bind_param("sssisii", $booking_no, $pickup_date, $dropoff_date, $bike_no, $username, $booking_date, $total_price);
        if (!$stmt->execute()) {
